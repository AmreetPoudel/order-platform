...
read -sp "RabbitMQ password: " RABBITMQ_PASSWORD; echo
read -p "RabbitMQ username: " RABBITMQ_USER
...
aws ssm put-parameter --name "/order-platform/rabbitmq-password" --value "$RABBITMQ_PASSWORD" --type SecureString --region "$REGION" --overwrite
aws ssm put-parameter --name "/order-platform/rabbitmq-user" --value "$RABBITMQ_USER" --type SecureString --region "$REGION" --overwrite
